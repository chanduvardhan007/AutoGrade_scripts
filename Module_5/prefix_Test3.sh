...
#sed -i '/Introduce the prefix: Error: Prefix was not found./d' expected_titles.txt

echo "$student_output" | grep -A 1 "removeFromShoppingCart" | sed 's/^[ \t]*//;s/[ \t]*$//' | grep "Book:" > student_remove_titles.txt 
